...
def huber_loss_torch(y_true, y_pred, delta: float = 1.0):
    """
    Huber loss implemented with PyTorch tensors.
    ...
    y_pred : torch.Tensor
        Predicted values
    delta : float
        Threshold between L1 and L2 loss
...
def mean_huber_loss(y_true, y_pred, delta: float = 1.0):
    """
    Mean Huber loss wrapper so the name is compatible with old TensorFlow code.
    ...
    torch.Tensor
        Scalar loss
    """
    return huber_loss_torch(y_true, y_pred, delta).mean()
...
class Agent:
    ...
    def _point_to_row_col(self, point):
        """Convert a point value to (row, col)"""
        return point // self._board_size, point % self._board_size
...
class DQNNet(nn.Module):
    """
    Simple convolutional network for the Snake agent.

    The architecture is close to the original Keras version:
    Conv -> Conv -> Conv -> Dense -> Dense(out).
    """
    ...
    def __init__(self, board_size: int, n_frames: int, n_actions: int):
        ...
        # Use a dummy tensor to figure out flattened size
        with torch.no_grad():
        ...
        self.fc = nn.Sequential(
            ...
            # No activation here because Q-values are linear outputs
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ----------
        x : torch.Tensor
            Input tensor of shape (batch, frames, board, board)
...
class DeepQLearningAgent(Agent):
    """This agent learns the game via Q-learning using PyTorch."""
    ...
    def __init__(self, board_size=10, frames=4, buffer_size=10000,
        ...
        """
        Initializer for DQN agent. Arguments are same as Agent class
        except we build PyTorch models and optimizers here.
        """
    ...
    def reset_models(self):
        """Reset all the models by creating new graphs."""
        self._model = DQNNet(self._board_size, self._n_frames, self._n_actions).to(self._device)
        if self._use_target_net:
            self._target_net = DQNNet(self._board_size, self._n_frames, self._n_actions).to(self._device)
            self.update_target_net()
        else:
    ...
    def _normalize_board(self, board: np.ndarray) -> np.ndarray:
        ...
        The original code scaled uint8 to smaller float values.
        Here we keep the same behaviour for compatibility.
        """
        return board.astype(np.float32) / 4.0

    def _prepare_input(self, board: np.ndarray) -> np.ndarray:
        """
        Reshape input and normalize.
        ...
        """
        if board.ndim == 3:
            # Add batch dimension if only one board is provided
            board = board.reshape((1,) + self._input_shape)
        # Normalize
        board = self._normalize_board(board.copy())
        return board.copy()

    def _numpy_to_torch(self, board: np.ndarray) -> torch.Tensor:
        """
        Convert board from numpy (B, H, W, C) to PyTorch tensor (B, C, H, W).
        """
        # (batch, board, board, frames) -> (batch, frames, board, board)
        board_tensor = torch.from_numpy(board).permute(0, 3, 1, 2).to(self._device)
        return board_tensor
    ...
    def _get_model_outputs(self, board: np.ndarray, model=None) -> np.ndarray:
        ...
        ----------
        board : Numpy array
            Board state(s) for which to predict action values
        model : nn.Module, optional
            Which network to use (main or target); defaults to main.

        Returns
        -------
        Numpy array of Q-values with shape (batch, n_actions).
        """
        # Prepare numpy input
        board_np = self._prepare_input(board)
        board_tensor = self._numpy_to_torch(board_np)
    ...
    def move(self, board, legal_moves, value=None):
        """
        Select the greedy action (argmax over Q-values) while respecting legal moves.
        ...
        legal_moves : Numpy array
            Mask of legal actions for each game in the batch
        value : None
            Kept for compatibility

        Returns
        -------
        Numpy array of chosen actions, shape (batch,)
        """
        q_values = self._get_model_outputs(board, self._model)
        # Mask out illegal moves by putting very low value
    ...
    def get_action_proba(self, board, values=None):
        """
        Get action probabilities using a softmax over Q-values.
        ...
        Returns
        -------
        Numpy array of probabilities, shape (batch, n_actions)
        """
        q_values = self._get_model_outputs(board, self._model)
        ...
        exps = np.exp(q_values)
        probs = exps / exps.sum(axis=1, keepdims=True)
        return probs

    def save_model(self, file_path="", iteration=None):
        """
        Save the current models to disk using torch.save.
        We keep the old '.h5' extension so file names match the original code.
        """
        if iteration is not None:
            assert isinstance(iteration, int), "iteration should be an integer"
        else:
        ...
        # Save main network
        torch.save(self._model.state_dict(), f"{file_path}/model_{iteration:04d}.h5")

        # Save target network if it exists
        if self._use_target_net and self._target_net is not None:
            torch.save(self._target_net.state_dict(), f"{file_path}/model_{iteration:04d}_target.h5")
    ...
    def load_model(self, file_path="", iteration=None):
        ...
        self._model.load_state_dict(torch.load(f"{file_path}/model_{iteration:04d}.h5",
                                               map_location=self._device))

        # Load target network weights if used
        if self._use_target_net and self._target_net is not None:
            self._target_net.load_state_dict(
                torch.load(f"{file_path}/model_{iteration:04d}_target.h5",
                           map_location=self._device)
            )
    ...
    def train_agent(self, batch_size=32, num_games=1, reward_clip=False):
        """
        Train the model by sampling from buffer and return the loss value.

        We follow the same idea as the original DQN code:
        target = r + gamma * max_a' Q_target(next_s, a')
        and update Q(s, a) towards this target using Huber loss.
        """
        ...
        # Prepare tensors
        s_np = self._prepare_input(s)
        next_s_np = self._prepare_input(next_s)

        s_tensor = self._numpy_to_torch(s_np)
        next_s_tensor = self._numpy_to_torch(next_s_np)

        r_tensor = torch.from_numpy(r.astype(np.float32)).to(self._device)
        done_tensor = torch.from_numpy(done.astype(np.float32)).to(self._device)
        legal_moves_tensor = torch.from_numpy(legal_moves.astype(np.float32)).to(self._device)
        a_tensor = torch.from_numpy(a.astype(np.float32)).to(self._device)

        # Compute target Q-values using target network or main network
        if self._use_target_net and self._target_net is not None:
        ...
        with torch.no_grad():
            q_next_all = target_model(next_s_tensor)  # (batch, n_actions)
            # Mask illegal moves by assigning large negative value
            mask = (legal_moves_tensor == 0)
            q_next_all = q_next_all.masked_fill(mask, float("-inf"))
            max_next_q, _ = torch.max(q_next_all, dim=1, keepdim=True)

            # Bellman target
            target_q = r_tensor + self._gamma * max_next_q * (1.0 - done_tensor)

        # Current Q-values from main network
        q_all = self._model(s_tensor)  # (batch, n_actions)
        # Select Q-values for actions that were actually taken
        q_selected = (q_all * a_tensor).sum(dim=1, keepdim=True)

        # Compute Huber loss
        loss = mean_huber_loss(target_q, q_selected)
    ...
    def compare_weights(self):
        """
        Utility function to check if the model and target network
        have the same weights. Mainly for debugging.
        """
        if not self._use_target_net or self._target_net is None:
            print("Target network is not in use.")
            return
        ...
        for (name1, param1), (name2, param2) in zip(
            ...
            if not torch.equal(param1, param2):
                print(f"Layer {name1} and {name2} do NOT match.")
                matches = False
        if matches:
            print("All layers match between model and target network.")

    def copy_weights_from_agent(self, agent_for_copy):
        """
        Copy weights from another agent of the same type.
        Useful when training multiple agents in parallel.
        """
        ...
        if self._use_target_net and self._target_net is not None and agent_for_copy._target_net is not None:
            self._target_net.load_state_dict(agent_for_copy._target_net.state_dict())


# --------------------------------------------------------------------------------------
# The following agents (PolicyGradientAgent, AdvantageActorCriticAgent, SupervisedLearningAgent)
# are not required for the basic Deep Q-learning assignment.
# We keep simple placeholders so that imports do not fail.
# --------------------------------------------------------------------------------------


class PolicyGradientAgent(Agent):
    """Placeholder for Policy Gradient agent.
    Not implemented for the PyTorch DQN assignment.
    """

    def __init__(self, *args, **kwargs):
        raise NotImplementedError("PolicyGradientAgent is not implemented in this solution.")


class AdvantageActorCriticAgent(Agent):
    """Placeholder for Advantage Actor-Critic agent.
    Not implemented for the PyTorch DQN assignment.
    """

    def __init__(self, *args, **kwargs):
        raise NotImplementedError("AdvantageActorCriticAgent is not implemented in this solution.")


class HamiltonianCycleAgent(Agent):
    """This agent prepares a Hamiltonian Cycle through the board and then
    follows it to reach the food, inherits Agent
    (unchanged from original TensorFlow version, since it is not NN-based).
    """

    def __init__(self, board_size=10, frames=4, buffer_size=10000,
                 gamma=0.99, n_actions=3, use_target_net=False,
                 version=""):
        assert board_size % 2 == 0, "Board size should be even for hamiltonian cycle"
        super().__init__(board_size=board_size, frames=frames, buffer_size=buffer_size,
                         gamma=gamma, n_actions=n_actions, use_target_net=use_target_net,
                         version=version)
        self._get_cycle_square()
    ...
    def _get_neighbors(self, point):
        """
        Get neighbors inside the inner board (excluding outer border).
        """
        row, col = point // self._board_size, point % self._board_size
        neighbors = []
        for delta_row, delta_col in [[-1, 0], [1, 0], [0, 1], [0, -1]]:
    ...
    def _get_cycle_square(self):
        """
        Simple implementation to get a Hamiltonian cycle
        for a square board, by traversing in an up and down fashion.
        All movement code is based on this implementation.
        """
        self._cycle = np.zeros(((self._board_size - 2) ** 2,), dtype=np.int64)
        index = 0
    ...
    def move(self, board, legal_moves, values):
        """Get action using Hamiltonian cycle policy."""
        cy_len = (self._board_size - 2) ** 2
        curr_head = np.sum(
            self._board_grid * (board[:, :, 0] == values["head"]).reshape(self._board_size, self._board_size)
        )
        index = 0
        while True:
            if self._cycle[index] == curr_head:
        ...
        if board[prev_head // self._board_size, prev_head % self._board_size, 0] == 0:
            ...
            prev_head_row, prev_head_col = self._point_to_row_col(prev_head)
            next_head_row, next_head_col = self._point_to_row_col(next_head)
            dx = next_head_col - curr_head_col
            dy = -next_head_row + curr_head_row
            if dx == 1 and dy == 0:
                return 0
            elif dx == 0 and dy == 1:
                return 1
...
class BreadthFirstSearchAgent(Agent):
    """
    Finds the shortest path from head to food while avoiding borders and body.
    Purely algorithmic, so it is unchanged from the original code.
    """
    ...
    def _get_shortest_path(self, board, values):
        ...
        while True:
            ...
            for p in n:
                row, col = self._point_to_row_col(p)
                if distances[row][col] != np.inf and distances[row][col] == distances[curr_row][curr_col] - 1:
                    path.append(p)
                    curr_point = p
                    break
        return path
    ...
    def move(self, board, legal_moves, values):
        ...
        a = np.zeros((board.shape[0],), dtype=np.uint8)
        for i in range(board.shape[0]):
            board_i = board_main[i, :, :, :]
            path = self._get_shortest_path(board_i, values)
            if len(path) == 0:
                a[i] = 1
                continue
            next_head = path[-2]
            curr_head = (self._board_grid * (board_i[:, :, 0] == values["head"])).sum()
            if (
                (board_i[:, :, 0] == values["head"])
                + (board_i[:, :, 0] == values["snake"])
                == (board_i[:, :, 1] == values["head"])
                + (board_i[:, :, 1] == values["snake"])
            ).all():
                prev_head = curr_head - 1
            else:
                prev_head = (self._board_grid * (board_i[:, :, 1] == values["head"])).sum()
            curr_head_row, curr_head_col = self._point_to_row_col(curr_head)
            prev_head_row, prev_head_col = self._point_to_row_col(prev_head)
            next_head_row, next_head_col = self._point_to_row_col(next_head)
            dx = next_head_col - curr_head_col
